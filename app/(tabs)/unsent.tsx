...
export default function UnsentTab() {
  ...
  const handleDelete = (conv: UnsentConversation) => {
    Alert.alert(
      'Delete Conversation',
      `Delete your conversation with "${conv.recipientName}"?`,
      [
  ...
  const renderItem = ({ item }: { item: UnsentConversation }) => {
    ...
    const emoji = RELATIONSHIP_OPTIONS.find((r) => r.key === item.relationshipType)?.emoji ?? '✨';
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: topPad + 8, backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        <View style={styles.headerLeft}>
          <Text style={[styles.title, { color: colors.navy }]}>Unsent</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={<EmptyState colors={colors} onNew={() => router.push('/unsent/new')} />}
...
function EmptyState({ colors, onNew }: { colors: any; onNew: () => void }) {
