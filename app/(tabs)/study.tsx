...
import {
  ...
  TextInput, Modal, Alert, Platform, Pressable,
} from 'react-native';
...
import {
  ...
  addStudySession, SessionType, CompletionStatus,
  addStudyDeadline, updateStudyDeadline, deleteStudyDeadline,
  StudyDeadline, DeadlineType, DeadlinePriority,
  addStudyNote, deleteStudyNote,
} from '@/lib/firestore';
...
export default function StudyBuddyScreen() {
  ...
  const saveSession = async () => {
    ...
    try {
      await addStudySession(user.uid, {
        ...
        goalText, sessionType, completionStatus, reflection,
        mood: '', date: today,
      });
    } catch { /* silent */ }
    closeTimer();
  ...
  const saveSubject = async () => {
    ...
    await addSubject(user.uid, {
      ...
      icon: newSubjectIcon, teacher: newSubjectTeacher.trim(), totalMinutes: 0,
    });
  ...
  const saveDeadline = async () => {
    ...
    await addStudyDeadline(user.uid, {
      ...
      dueDate: new Date(y, m-1, d), priority: dlPriority, status: 'pending',
    });
  ...
  const toggleDeadlineDone = async (dl: StudyDeadline) => {
    if (!user) return;
    await updateStudyDeadline(user.uid, dl.id, { status: dl.status === 'done' ? 'pending' : 'done' });
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 10, backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        <Text style={[styles.headerTitle, { color: colors.navy }]}>📚 Study Buddy</Text>
        <TouchableOpacity
          onPress={() => {
      ...
      >
        ...
        {tab === 'home' && (
          <>
            ...
            <TouchableOpacity style={[styles.startBtn, { backgroundColor: colors.primary }]} onPress={() => setShowTimer(true)}>
          ...
          <>
            {subjects.length === 0 ? (
              ...
              <View style={styles.subjectGrid}>
                {subjects.map(s => {
                  const subMins = sessions.filter(ss=>ss.subjectId===s.id).reduce((a,ss)=>a+ss.durationMinutes,0);
                  return (
                    <TouchableOpacity
                      key={s.id}
          ...
          {timerPhase === 'active' && (
            <View style={styles.timerActive}>
              ...
              <View style={styles.timerControls}>
                <TouchableOpacity style={styles.timerControlBtn} onPress={() => setTimerPaused(p => !p)}>
                ...
                {(['yes','partial','no'] as CompletionStatus[]).map(c => (
                  <TouchableOpacity
                    ...
                    style={[styles.completionBtn, {
                      backgroundColor: completionStatus===c ? (c==='yes' ? '#5DB87A' : c==='partial' ? '#F5A555' : '#FF6B6B') : colors.surfaceAlt,
